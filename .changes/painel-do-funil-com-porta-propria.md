...
movimento que nunca houve. O beacon não guarda IP nem e-mail: país, cidade e
CEP vêm dos cabeçalhos da borda, e a coluna é **CEP**, não "bairro" — a borda
não entrega bairro, e prometer precisão que o dado não tem faz decidir sobre
ruído.

**O painel tem porta própria em vez de usar o login do produto.** O que se lê
ali não pertence a organização nenhuma, e pendurá-lo na sessão do produto
entregaria a lista de clientes a quem viesse a ser platform admin por engano.
A proteção inteira é `lib/painel/sessao.ts`, que falha fechada em três degraus.
...
24 caracteres é o que torna a assinatura inviável de forjar offline por quem já
tem um cookie válido. Senha curta não liga o painel. Quem passa do limite de dez
tentativas por IP recebe o **mesmo** 401 de quem errou a senha, sem
`Retry-After`: dizer quanto falta transforma o limite num relógio a esperar.
