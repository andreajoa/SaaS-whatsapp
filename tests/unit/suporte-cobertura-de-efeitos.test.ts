...
function files(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true })
    .flatMap((item) => (item.isDirectory() ? files(join(dir, item.name)) : [join(dir, item.name)]))
    .map(emBarraNormal);
}
...
it("todo handler mutante do app declara guarda de suporte ou é infraestrutura identificada", () => {
  ...
  for (const path of files("app/api/v1").filter((p) => p.endsWith("/route.ts"))) {
    if (/app\/api\/v1\/(cron|webhooks)\//.test(path) || path === "app/api/v1/system/agent/route.ts")
      continue; // segredo de máquina, sem actor/session cookie
    if (path.includes("/impersonate")) continue; // início/fim autenticam a posse e têm contrato próprio
    // O SITE PÚBLICO E A PORTA DO PAINEL: não há tenant, e por isso não há o que
    // guardar. `requireSupportWrite()` responde UMA pergunta — "este write está
    // sendo feito por um admin de plataforma dentro da organização de outro?" —
    // e ela pressupõe uma organização resolvida e um ator com sessão. Quem
    // chama `/site/lead`, `/site/visita`, `/site/contato` e `/site/descadastrar`
    // é um ESTRANHO no navegador, antes de existir conta; `/painel/entrar` é a
    // porta de senha do próprio painel, e exigir sessão nela seria exigir estar
    // dentro para poder entrar.
    //
    // Pôr a guarda aqui não deixaria nada mais seguro: sem `activeOrg` ela
    // resolve para "não é suporte" e libera, sempre. O que de fato protege estas
      ...
      if (ts.isVariableStatement(node))
        for (const decl of node.declarationList.declarations)
          if (
            ...
            ["POST", "PUT", "PATCH", "DELETE"].includes(decl.name.text) &&
            decl.initializer
          )
...
it("Server Actions que resolvem tenant declaram efeito ou uma exceção pessoal/transição", () => {
  ...
  const uncovered = files("app/actions")
    .filter((p) => !p.endsWith(".test.ts") && !exceptions.has(p.split("/").at(-1)!))
    .filter((p) => {
